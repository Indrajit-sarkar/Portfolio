# Indrajit Sarkar — Portfolio Knowledge Base

## Personal Information
- **Full Name**: Indrajit Sarkar
- **Current Role**: Analyst II · Infrastructure Management at DXC Technology
- **Title**: Azure AI Engineer & Full-Stack Developer
- **Status**: Open to opportunities
- **Location**: Bengaluru, Karnataka 560032, India
- **Primary Email**: sarkar.sarkar.indrajit01@gmail.com
- **Secondary Email**: indusakar01@gmail.com
- **Primary Phone**: +91 86188 35953
- **Secondary Phone**: +91 78998 18522
- **LinkedIn**: https://linkedin.com/in/indrajit-sarkar01
- **GitHub**: https://github.com/Indrajit-sarkar
- **Instagram**: https://www.instagram.com/indrajit.sarkar.1293/
- **X (Twitter)**: https://x.com/Indrajit42680107
- **Portfolio Website**: https://indrajit-sarkar.vercel.app

## Summary
Results-driven Infrastructure Management professional and Microsoft Certified Azure AI Engineer with 2+ years of experience at DXC Technology. Proven track record in cloud infrastructure optimization, IT service delivery, and process automation using Azure, Python, and Power BI. Holds **67 professional certifications** spanning cloud computing (Microsoft Azure), AI/ML (Anthropic Claude), Lean Six Sigma (Black Belt), Agile project management, and process excellence. Adept at driving SLA compliance (98%), reducing operational costs (15%), and accelerating incident resolution (35%). Published independent researcher in Agentic AI and Quantum Computing.

**Core Competencies**: Azure Cloud Services · Infrastructure Monitoring · Python Automation · Lean Six Sigma Black Belt · OPEX · ITIL · Power BI Analytics · Full-Stack Development · Agile/Scrum · FMEA · Root Cause Analysis · Process Optimization · CI/CD · Database Management

---

## Education

### 1. Master of Computer Applications (MCA)
- **Institution**: Atria Institute of Technology, Bengaluru
- **Website**: https://atria.edu
- **Duration**: February 2024 — November 2025
- **Status**: Completed (2025)

### 2. Bachelor of Computer Applications (BCA)
- **Institution**: Indian Academy Degree College (Autonomous), Bengaluru
- **Website**: https://iadc.ac.in
- **Duration**: August 2020 — November 2023
- **Status**: Completed

### 3. Pre-University (PUC) — CEBA
- **Stream**: Commerce, Economics, Business Studies, Accountancy
- **Institution**: UAS and VC PU College, Bengaluru
- **Website**: https://www.uasbangalore.edu.in
- **Duration**: 2018 — 2020
- **Status**: Completed

### 4. Schooling
- **Institution**: Allwin Public School, Bengaluru
- **Passout Year**: 2018
- **Distinction**: School Captain

---

## Professional Experience

### 1. DXC Technology (dxc.com)
- **Role**: Analyst II · Infrastructure Management
- **Location**: Bengaluru
- **Duration**: January 2024 — Present
- **Key Responsibilities**:
  - Managed end-to-end infrastructure monitoring and maintenance for multiple enterprise clients, consistently maintaining 98% SLA compliance across 200+ monthly technical incidents.
  - Automated routine infrastructure tasks using Python scripts and Azure technologies, reducing manual intervention time by 40% across 3 client accounts.
  - Optimized Azure cloud infrastructure for digital transformation, achieving 15% cost reduction and 25% improvement in system response times.
  - Collaborated with cross-functional teams of 8+ members to resolve complex incidents; implemented monitoring solutions to identify bottlenecks.
  - Delivered client-centric infrastructure upgrades enhancing service quality by 20%; created knowledge base docs reducing ticket resolution by 35%.
  - Applied Lean Six Sigma Black Belt methodologies (FMEA, RCA, QFD, Kano) for continuous process improvement.
- **Key Metrics**:
  - 40% Automation Improvement
  - 200+ Incidents resolved per month
  - 98% SLA Compliance Rate
  - 15% Infrastructure cost reduction
  - 25% System response time improvement
  - 35% Ticket resolution time reduction
- **Technologies**: Azure, Python, Power BI, ServiceNow, PowerShell, ITIL

### 2. Talent Brainy
- **Role**: Full-Stack Web Development Intern
- **Duration**: 2023
- **Key Responsibilities**:
  - Developed and deployed 2 responsive web applications using HTML5, CSS3, JavaScript, and PHP for internal clients.
  - Engineered MySQL database schemas with optimized indexing, improving query performance by 40%.
  - Integrated RESTful APIs enhancing application responsiveness by 30%.
  - Delivered milestones 15% ahead of schedule in a 4-developer Agile team.
- **Key Metrics**:
  - 40% Query Performance Improvement
  - 30% Application Responsiveness Improvement
  - 15% Ahead of Schedule delivery
  - 100% Project Completion
- **Technologies**: HTML5, CSS3, JavaScript, PHP, MySQL, RESTful APIs, Agile

---

## Projects (9 Total)

### Featured Projects

1. **NAS Solution Guide**
   - **Tech Stack**: HTML, CSS, JavaScript
   - **Description**: A comprehensive, step-by-step resource for building a powerful private Network-Attached-Storage server — from repurposing an old laptop to configuring a Raspberry Pi, and taking full control of your data.
   - **Live Demo**: https://nas-solution.vercel.app/
   - **GitHub**: https://github.com/Indrajit-sarkar/NAS-solution

2. **Image Recognition**
   - **Tech Stack**: Python, YOLOv8, OpenCV
   - **Description**: High-performance image recognition with cloud API integration — real-time object detection, currency recognition, people detection and gesture-based cursor control, built with YOLOv8, MediaPipe and Google Cloud Vision.
   - **GitHub**: https://github.com/Indrajit-sarkar/Image-Recognition

3. **AI Data Extraction**
   - **Tech Stack**: Python, Azure Document Intelligence, OpenAI API
   - **Description**: Enterprise-grade document processing combining intelligent PDF splitting with advanced data extraction — built on Azure AI services and optimized for high-performance parallel processing.
   - **GitHub**: https://github.com/Indrajit-sarkar/AI-Data-extraction-and-PDF-split

4. **Paris Tourism**
   - **Tech Stack**: PHP, HTML, CSS, JavaScript
   - **Description**: Tourism website covering Paris — from the golden glow of the Seine at sunset to the hidden secrets of Montmartre.
   - **GitHub**: https://github.com/Indrajit-sarkar/Paris-Tourism

5. **Scientific Calculator**
   - **Tech Stack**: HTML, CSS, JavaScript
   - **Description**: A premium scientific calculator that replicates the look and feel of native iOS apps — zero dependencies, no frameworks, no build tools.
   - **GitHub**: https://github.com/Indrajit-sarkar/Scientific-Calculator

### Additional Projects

6. **Money Manager**
   - **Tech Stack**: PHP, HTML, CSS, JavaScript, SQL
   - **GitHub**: https://github.com/Indrajit-sarkar/Money-manager

7. **AI Assistant**
   - **Tech Stack**: Python, HTML, CSS, JavaScript
   - **GitHub**: https://github.com/Indrajit-sarkar/AI-Assistant

8. **Sentiment Analyzer**
   - **Tech Stack**: Python, HTML, CSS, JavaScript
   - **GitHub**: https://github.com/Indrajit-sarkar/Sentiment-Analyzer-

9. **ARCADE (Game Selector)**
   - **Tech Stack**: HTML, CSS, JavaScript
   - **Description**: Game arcade featuring Sketch Ops (2D jetpack combat) and VoxelCraft (Minecraft-inspired sandbox). Includes loading screens, rules display, mobile touch controls, and landscape orientation enforcement.
   - **GitHub**: https://github.com/Indrajit-sarkar/ARCADE
   - **Live**: Available at /games.html on the portfolio site

---

## Technical Skills

### Programming Languages
- Java — High-level, class-based, object-oriented language
- Python — High-level, interpreted language for web dev, data science, and AI
- C++ — General-purpose language supporting procedural, OOP, and generic programming
- C# — Modern object-oriented language by Microsoft for .NET
- JavaScript — Web language for interactive pages and modern web apps
- PHP — Server-side scripting language for web development
- Shell Scripting — Automation and system management via Bash

### Cloud & AI
- Microsoft Azure — Cloud platform (compute, analytics, storage, networking)
- Azure AI Services — Pre-built AI for vision, speech, language, decision-making
- Azure OpenAI — GPT-4 and language models in secure Azure environment
- Machine Learning — Computers learning from data and making predictions
- Computer Vision — Machines interpreting visual information from images/video
- Natural Language Processing (NLP) — Understanding and generating human language
- Azure Cognitive Services — Pre-trained AI models

### Methodologies
- Agile — Iterative project management approach
- Scrum — Agile framework for team collaboration
- FMEA — Failure Mode and Effects Analysis
- Operational Excellence (OPEX) — Systematic approach to improving business performance
- Six Sigma — Data-driven methodology for eliminating defects
- DMAIC — Define, Measure, Analyze, Improve, Control
- Lean — Minimizing waste and maximizing value
- Kaizen — Continuous improvement philosophy
- 5S — Sort, Set in Order, Shine, Standardize, Sustain

### Tools & Web Technologies
- HTML5, CSS3 — Web markup and styling
- React — JavaScript UI library
- MySQL — Relational database
- MongoDB — NoSQL document database
- Power BI — Business analytics and data visualization
- Minitab — Statistical analysis software
- Git & GitHub — Version control and collaboration
- VS Code — Code editor
- VMware — Virtualization
- Docker — Containerization platform
- TrueNAS — Network-attached storage OS

---

## Certifications (67 Total)

### Microsoft Certifications (3)
1. **AI-102: Azure AI Engineer Associate** — Microsoft (Oct 2025)
2. **PL-300: Power BI Data Analyst Associate** — Microsoft (Oct 2025)
3. **AZ-900: Azure Fundamentals** — Microsoft (Oct 2025)

### Anthropic — Claude Certified (4)
4. **Claude Certified Architect — Professional** — Anthropic (Sep 2026, Credly verified)
5. **Claude Certified Architect — Foundations** — Anthropic (Aug 2026)
6. **Claude Certified Developer — Foundations** — Anthropic (Aug 2026)
7. **Claude Certified Associate — Foundations** — Anthropic (Aug 2026)

### AIGPE Platform Certifications (14)
8. **AIGPE Lean Six Sigma Black Belt** — AIGPE (Oct 2025)
9. **AIGPE Lean Six Sigma Green Belt** — AIGPE (Oct 2025)
10. **AIGPE Lean Six Sigma Yellow Belt** — AIGPE (Oct 2025)
11. **AIGPE Lean Six Sigma White Belt** — AIGPE (Nov 2024)
12. **AIGPE Certified QFD Specialist** — AIGPE (Oct 2025)
13. **AIGPE Certified 8D Problem Solving Expert** — AIGPE (Oct 2025)
14. **AIGPE Certified Kano Analysis Specialist** — AIGPE (Feb 2026)
15. **AIGPE Certified Pareto Analysis Specialist** — AIGPE (Oct 2025)
16. **AIGPE Certified Root Cause Analysis Specialist** — AIGPE (Feb 2026)
17. **AIGPE Certified Five Forces Analysis Specialist** — AIGPE (Oct 2025)
18. **AIGPE Certified Minitab Beginner** — AIGPE (Oct 2025)
19. **AIGPE Certified FMEA Specialist** — AIGPE (Oct 2025)
20. **AIGPE Certified Process Mapping Specialist** — AIGPE (Sep 2026)
21. **AIGPE Certified Brainstorming Expert** — AIGPE (Sep 2026)

### Six Sigma Academy Amsterdam — SSAA (9)
22. **Lean Six Sigma Black Belt** — SSAA (Oct 2025)
23. **Certified Lean Six Sigma Green Belt** — SSAA (Sep 2026)
24. **Certified Process Improvement & Process Mapping Expert** — SSAA (Sep 2026)
25. **Certified Risk Management FMEA ISO 31000 Expert** — SSAA (Sep 2026)
26. **Certified Lean Management & Manufacturing Expert** — SSAA (Sep 2026)
27. **Certified Agile Scrum Project Management Expert** — SSAA (Sep 2026)
28. **Certified AI Fundamentals Professional** — SSAA (Sep 2026)
29. **Minitab Data Analyst Expert** — SSAA (Sep 2026)
30. **Lean Six Sigma — Six Sigma Academy Amsterdam** — SSAA (2025)
31. **Certified Operational Excellence (OPEX) Expert** — SSAA (Sep 2026)

### Udemy Certifications (27)
32. **Six Sigma: Certified Lean Six Sigma Green Belt (Accredited)** — Udemy / SSAA (Sep 2026)
33. **Six Sigma: Certified Lean Six Sigma Green Belt (Accredited)** — Udemy / AIGPE (Sep 2026)
34. **Six Sigma Black Belt: Certified QFD Specialist (Accredited)** — Udemy / AIGPE (Sep 2026)
35. **Six Sigma Green Belt: Kano Analysis Specialist (Accredited)** — Udemy / AIGPE (Sep 2026)
36. **Certified Lean Management Expert (Accredited)** — Udemy / SSAA (Sep 2026)
37. **Lean Management: Just-In-Time JIT Certification (Accredited)** — Udemy / SSAA (Sep 2026)
38. **Certified Process Improvement & Process Mapping Expert** — Udemy / SSAA (Sep 2026)
39. **Process Mapping & Flowcharts Certification (Accredited)** — Udemy / AIGPE (Sep 2026)
40. **Certified Risk Management FMEA ISO 31000 Expert (Accredited)** — Udemy / SSAA (Sep 2026)
41. **FMEA Training: FMEA Specialist Certification (Accredited)** — Udemy / AIGPE (Sep 2026)
42. **Certified Five Forces Analysis Specialist (Accredited)** — Udemy / AIGPE (Sep 2026)
43. **Pareto Analysis Masterclass: Pareto Specialist (Accredited)** — Udemy / AIGPE (Sep 2026)
44. **Minitab Data Analysis: Certified Minitab Expert (Accredited)** — Udemy / SSAA (Sep 2026)
45. **Certified Minitab Beginner: Graphical Tools (Accredited)** — Udemy / AIGPE (Sep 2026)
46. **Certified 8D Problem Solving Expert — 8D for All Industries** — Udemy / SSAA (Sep 2026)
47. **AI-Powered Root-Cause Analysis Specialist (Accredited)** — Udemy / AIGPE (Sep 2026)
48. **Certified Brainstorming Expert Masterclass (Accredited)** — Udemy / AIGPE (Sep 2026)
49. **Certified Agile Scrum Project Management Expert (Accredited)** — Udemy / SSAA (Sep 2026)
50. **Certified Artificial Intelligence Foundations Professional** — Udemy / SSAA (Sep 2026)
51. **Prompt Engineering for Work** — Udemy (Sep 2026)
52. **Windows 11: Beginner to Advanced** — Udemy (Sep 2026)
53. **The Fundamentals of Resilience** — Udemy (Sep 2026)
54. **Creativity, Problem Solving and Generating Alternatives** — Udemy (Sep 2026)
55. **Time and Task Management Techniques** — Udemy (Sep 2026)
56. **Critical Thinking with Emotional Intelligence** — Udemy (Sep 2026)
57. **Emotional Intelligence: Everyday Empathy at Work** — Udemy (Sep 2026)
58. **The Harmonious Workplace** — Udemy (Sep 2026)

### LinkedIn Learning Certifications (4)
59. **Customer Service Call Control Strategies** — LinkedIn Learning (Jan 2024)
60. **How to Be Both Assertive and Likable** — LinkedIn Learning (Jan 2024)
61. **IT Service Desk: Customer Service Fundamentals** — LinkedIn Learning (Jun 2025)
62. **IT Service Desk: Service Management** — LinkedIn Learning (Sep 2025)

### Other Professional Certifications (5)
63. **AIGPE Certified Lean Just-In-Time Specialist** — AIGPE (Oct 2025)
64. **Google Cloud Computing Foundations** — Google (2023)
65. **Cyber Security Certification** — Edify Institute (2023)
66. **Internship Certificate — Full Stack Development** — Talent Brainy (2023)
67. **AIGPE Certified Six Sigma White Belt Badge** — AIGPE (badge/digital credential)

### Certification Summary by Issuer
| Issuer | Count | Categories |
|--------|-------|------------|
| Microsoft | 3 | Azure AI, Power BI, Azure Fundamentals |
| Anthropic | 4 | Claude Architect (Pro + Foundations), Developer, Associate |
| AIGPE | 14 | Six Sigma Belts (Black/Green/Yellow/White), Process Tools (QFD, 8D, Kano, Pareto, RCA, Five Forces, Minitab, FMEA, Process Mapping, Brainstorming) |
| SSAA (Six Sigma Academy Amsterdam) | 10 | Six Sigma Belts, Process Improvement, Risk Management, Lean Manufacturing, Agile Scrum, AI Fundamentals, Minitab |
| Udemy | 27 | Accredited Six Sigma, Process Excellence, AI/ML, Soft Skills, Productivity |
| LinkedIn Learning | 4 | Customer Service, IT Service Desk, Communication |
| Other (Google, Edify, Talent Brainy) | 5 | Cloud Computing, Cyber Security, Internship, JIT |

---

## Achievements
1. **1st Prize — SHRISTI State-Level Coding Championship**: Awarded by Dr. S. Somanath, Chairman of ISRO.
2. **1st Prize — GenZ Quiz**: Inter-college event VIGYAANRANG.
3. **2nd Prize — TECHNOBINGE 2K21**: National Level Collegiate Fest (EVENTILIA).
4. **2nd Prize — Techforte-2K21**: Inter-college Championship, Presentation on Robotics.
5. **Core Student Coordinator**: Inter-collegiate Fest CUEST-2K24.
6. **School Captain**: Led student council and organized events at Allwin Public School.
7. **Champion — Inter-school Kabaddi and Football Championships**.
8. **Champion — Intra-college Quiz, Debate, and Poster Designing competitions**.

---

## Volunteer & Community Activities
1. **Diabetes Awareness Camp**: Coordinated for Neuberg Anand Reference Laboratory, reaching 500+ participants.
2. **CLT India Volunteer**: Community-driven program empowering local communities through education, healthcare, and sustainable development.
3. **Blood Donation Camp**: Volunteered in organizing and participation.
4. **Reforestation Awareness Program (IADCA)**: Environmental sustainability volunteer.
5. **Speech on AI Importance**: Explored how AI is revolutionizing industries and shaping the future of technology.
6. **Effects of Digital Crime Talk**: Spoke on consequences of cybercrime and strategies for digital safety.
7. **Skit on Farmers' Importance**: Theatrical performance highlighting farmers' vital role in national food security.
8. **Visit to Grace Foundation**: Community outreach connecting with underprivileged communities.
9. **Grocery & Clothes Donation**: Organized and participated in distributing essential supplies to families in need.
10. **Active member**: Computer Club, Debate Club, and Student Council.

---

## Research Publications

### 1. Agentic AI Between Capability and Reliability (August 2026)
- **Author**: Indrajit Sarkar (Independent Researcher)
- **ORCID**: https://orcid.org/0009-0000-3038-6379
- **Type**: Review Article (60 pages)
- **Topic**: Comprehensive structured review of autonomous AI agent systems built on large language models
- **Key Findings**:
  - Capability is no longer the scarce resource — reliability under composition and governance under autonomy are the binding constraints
  - On curated benchmarks, agents approach or exceed human baselines (SWE-bench: ~20% to ~80% solved in under a year)
  - On live interfaces and extended horizons, the same systems complete only 7–50% of tasks
  - Proposes a Capability-Reliability-Governance framework for evaluating agentic AI systems
  - Introduces a minimum reporting standard for agent evaluation
  - Six falsifiable propositions for the research agenda through 2030
- **Topics Covered**: Reference architecture for agentic systems, design patterns, memory/planning/orchestration subsystems, multi-agent systems, tool use (Model Context Protocol), security/privacy/alignment, deployment economics, workforce transformation, interoperability protocols
- **Keywords**: Agentic AI, large language model agents, multi-agent systems, agent evaluation, long-horizon reliability, tool use, Model Context Protocol, AI governance, IT service management, workforce transformation
- **Link**: Available on Indrajit's portfolio under Research section

### 2. Quantum Computing at the Threshold of Utility (August 2026)
- **Author**: Indrajit Sarkar (Independent Researcher)
- **Type**: Review Article (25 pages) — Quantum Information Science and Technology
- **Topic**: Critical review of quantum computing hardware, algorithms, cryptographic implications, and commercial readiness (2024–2026)
- **Key Findings**:
  - Scientific quantum advantage is now demonstrable but economic advantage is not
  - The interval between the two is governed by logical error rates, compilation overheads, and availability of trained personnel
  - Superconducting processors report error per layered gate of ~2.15 × 10⁻³ at 100-qubit scale
  - Trapped-ion systems achieved 99.921% two-qubit fidelity
  - Neutral-atom arrays crossed the thousand-qubit line with coherence times approaching a minute
  - Post-quantum cryptographic standards (NIST) are now being finalized
  - At least one major financial institution withdrew from quantum research after resource estimation showed target problem beyond plausible machines
- **Topics Covered**: Six physical qubit modalities, algorithmic families and limitations, software/cloud ecosystems, cryptographic migration timeline, error-correction progress (surface codes to qLDPC), market structure and investment
- **Keywords**: Quantum computing, qubit modalities, quantum error correction, fault tolerance, variational algorithms, post-quantum cryptography, quantum advantage, technology readiness
- **Link**: Available on Indrajit's portfolio under Research section

---

## Domain Expertise

| Domain | Specific Area |
|--------|---------------|
| Cloud & Infrastructure | Azure Infrastructure Management, Cloud Optimization, SLA Management, Cost Reduction |
| Quality & Process | Lean Six Sigma (Black Belt), Operational Excellence (OPEX), FMEA, Root Cause Analysis, QFD, Kano Analysis, 8D Problem Solving, Pareto Analysis, Lean JIT |
| Software Development | Full-Stack Web Applications, Database Design, RESTful APIs, Agile/Scrum Methodology |
| Data & Analytics | Power BI Dashboards, Minitab Statistical Analysis, Data-Driven Decision Making |
| AI & Machine Learning | Azure AI Services, Computer Vision, NLP, Prompt Engineering |

---

## Hobbies & Interests
- **Automobiles** — Automotive research and analysis, car enthusiasm
- **Mobile Photography** — Capturing moments with smartphone cameras
- **Gaming** — Sketch Ops (2D jetpack combat), VoxelCraft (Minecraft-inspired sandbox) — playable on the portfolio site at /games.html
- **Plane Spotting** — Tracking aircraft, identifying airframes, photographing planes on approach/departure. Uses FlightRadar24 (flightradar24.com) for real-time flight tracking
- **Superyacht Enthusiasts** — Following superyacht launches, sea trials, shipyard builds, and interior reveals. Fascinated by the intersection of naval architecture, engineering, and luxury lifestyle
- **Music Production** — Creative audio projects
- **Sports** — Football, Kabaddi, Cricket
- **Research & Writing** — Published independent researcher in AI and Quantum Computing

---

## Additional Information
- **Languages**: English (Fluent), Hindi (Fluent), Kannada (Fluent), Bengali (Native)
- **Visa**: Indian Citizen
- **Portfolio Certifications Page**: All 66 certifications viewable with filters at /certifications.html
- **Resume PDF**: Downloadable at /resume.pdf

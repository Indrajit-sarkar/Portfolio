...
(() => {
    ...
    /* ---------- Intro curtain: "Designed by Indrajit" ---------- */
    function initIntro() {
        const intro = $('#intro');
        if (!intro) return;

        // shown once per browsing session, so moving between pages isn't a toll booth
        if (doc.classList.contains('intro-done')) { intro.remove(); return; }
        ...
        const timer = setTimeout(finish, reduced ? 500 : 2400);
    ...
    /* ---------- Portrait: colour follows the cursor ---------- */
    function initPortraitReveal() {
        const wrap = $('#portrait');
        if (!wrap) return;

        // no cursor to follow — CSS already shows the colour portrait outright
        if (!finePointer) return;

        let tx = 50, ty = 45, cx = 50, cy = 45, raf = 0, lit = false;
        ...
        wrap.addEventListener('pointermove', (e) => {
            ...
            if (!lit) {
                // land the spotlight where the cursor entered instead of sliding in from centre
                lit = true;
                cx = tx; cy = ty;
                wrap.style.setProperty('--mx', cx.toFixed(2) + '%');
                wrap.style.setProperty('--my', cy.toFixed(2) + '%');
    ...
    function initGsap() {
        ...
        // Transform only — animating opacity/scale on the stage art forces the
        // masked, filtered portrait to re-rasterise on every scroll frame.
        gsap.to('.stage-name', {
